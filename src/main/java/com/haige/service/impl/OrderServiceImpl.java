...
public class OrderServiceImpl implements OrderService {
  ...
  /** 订单表 */
  @Autowired private OrderDOMapper orderDOMapper;

  /** 套餐表 */
  @Autowired private GoodsInfoDOMapper goodsInfoDOMapper;
  /** 优惠券表 */
  @Autowired private GoodsCouponDOMapper goodsCouponDOMapper;

  @Autowired private IdWorker idWorker;
  @Autowired private UserBaseDOMapper userBaseDOMapper;

  @Autowired private WXPayService wxPayService;

  @Autowired private UserBaseService userBaseService;

  @Autowired private SmsClient smsClient;
  @Autowired private ServeDetailDOMapper serveDetailDOMapper;
  ...
  @Override
  public Mono<ResultInfo<List<OrderDetailsVO >>> queryOrderListByCondition(
    ...
    List<OrderDetailsVO> orderDetailsVOList = new ArrayList<>(orderDOList.size());

    for (int i = 0; i < orderDOList.size(); i++) {
      OrderDetailsVO orderDetailsVO = new OrderDetailsVO();

      orderDetailsVO.setGoodsId(orderDOList.get(i).getGoodsId().toString());

      orderDetailsVO.setOrderId(orderDOList.get(i).getOrderId());

      orderDetailsVO.setGoodsName(orderDOList.get(i).getGoodsName());

      orderDetailsVO.setPrice(orderDOList.get(i).getOrderPrice());
  ...
  @Override
  public Mono<ResultInfo<List<HashMap<String, String>>>> countOrder(
   ...
   */
  @Override
  public Mono<ResultInfo<List<OrderDetailsVO>>> queryOrderListByDetailsStatus(
    ...
    List<HashMap<String, String>> serveDetailDOList = serveDetailDOMapper
    ...
    ResultInfo<List<OrderDetailsVO>> result = new ResultInfo<List<OrderDetailsVO>>();
    result.setData(orderDetailsVOList);
    result.setCount(String.valueOf(orderDetailsVOList.size()));
    result.setCode(StatusCodeEnum.OK.getCode());
    result.setMessage(StatusCodeEnum.OK.getValue());
    return Mono.justOrEmpty(result);
