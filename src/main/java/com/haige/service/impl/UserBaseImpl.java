...
public class UserBaseImpl implements UserBaseService {
  ...
  @Autowired private UserBaseDOExtendMapper userBaseDOExtendMapper;

  @Autowired private GoodsCouponDOMapper goodsCouponDOMapper;
  @Autowired private WXLoginService wxLoginService;

  @Autowired private CouponDOMapper couponDOMapper;
  ...
  /** 查询用户的优惠券 */
  @Override
  public Mono<ResultInfo<List<CouponDO>>> userCoupon(ServerWebExchange serverWebExchange) {
    ServerHttpRequest request = serverWebExchange.getRequest();
  ...
  public Mono<ResultInfo> update(Mono<UpdateUserDTO> userBaseDTO) {
    return userBaseDTO
        .map(UserBaseConvertUtils::toVO)
        .doOnNext(userBaseDO -> userBaseDOMapper.updateByPrimaryKeySelective(userBaseDO))
